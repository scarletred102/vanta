...
single-process allocator. It also provides unbuffered C stream wrappers over
Vanta descriptors. It does not claim to be a complete libc, thread runtime,
environment, or POSIX implementation yet.
...
and unbuffered stdio smoke samples during native acceptance and requires their
success markers.

The next SDK steps are buffered `FILE` semantics, environment, and broader
process runtime support suitable for relibc rather than only direct wrappers.
