...
pub extern "C" fn _start() -> ! {
    ...
    loop {
        ...
        if length == b"register".len() as u64 && &request[..length as usize] == b"register" {
            let _ = vanta_userland::ipc_send(SEND_FD, b"audit:service-registered");
            continue;
        }
        if length == b"healthy".len() as u64 && &request[..length as usize] == b"healthy" {
            let _ = vanta_userland::ipc_send(SEND_FD, b"audit:service-upgraded");
